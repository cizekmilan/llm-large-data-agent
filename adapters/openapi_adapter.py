...
class OpenAPIAdapter:
    ...
    def get_tools(self):
        # naètu si openapi.json schéma
        spec = requests.get(self.openapi_url, timeout=10).json()

        tools = []        # definice funkcí pro LLM (neobsahuje vylouèené parametry)
        operations = {}   # metadata pro executor (plná informace, vèetnì pagination/meta)

        # parametry, které nechci vystavovat LLM (interní øízení)
        ...
        for path, methods in spec.get("paths", {}).items():
            for method, op in methods.items():
                ...
                # 1) NASBÍRÁNÍ PARAMETRÙ (query, path, body)
                properties = {}
                required = []
                ...
                # 3) FILTRACE PARAMETRÙ PRO LLM - LLM nesmí vidìt pagination ani meta_only
                llm_properties = {
                ...
                # 5) OPERACE PRO EXECUTOR - vèetnì rozšiøujících metadat
                operations[name] = {
                    "method": method.upper(),
                    "path": path,
